...
def Webcam(imagem):
    ...
    maskB = cv2.inRange(imagem_hsv, imagem_lower_hsvAzul, imagem_upper_hsvAzul)

    imagem_lower_hsvVermelho1 = np.array([0, 120, 100])  
    imagem_upper_hsvVermelho1 = np.array([10, 255, 255])
    mask1 = cv2.inRange(imagem_hsv, imagem_lower_hsvVermelho1, imagem_upper_hsvVermelho1)

    imagem_lower_hsvVermelho2 = np.array([170, 120, 100])  
    imagem_upper_hsvVermelho2 = np.array([180, 255, 255])
    mask2 = cv2.inRange(imagem_hsv, imagem_lower_hsvVermelho2, imagem_upper_hsvVermelho2)

    maskR = cv2.bitwise_or(mask1, mask2)
    maskFinal = cv2.bitwise_or(maskR, maskB)
